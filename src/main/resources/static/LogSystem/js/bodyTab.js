...
layui.define(["element","jquery"],function(exports){
    ...
    Tab.prototype.navBar = function(strData){
        ...
        navs = data;
        $("#topnav").find("cite").html(data[0].title);
        $("#iframeFirst").html('<iframe src="'+data[0].href+'"></iframe>');
        var ulHtml = '';
            ...
            if(data[i].children != undefined && data[i].children.length > 0){
                ...
                if(data[i].icon != undefined && data[i].icon != ''){
                    if(data[i].icon.indexOf("icon-") != -1){
                        ulHtml += '<i class="fa ' + data[i].icon +'"></i>';
                    }else{
                        ulHtml += '<i class="fa ' + data[i].icon +'"></i>';
                    }
                }
...
//                          ulHtml += '<i class="seraph '+data[i].children[j].icon+'" data-icon="'+data[i].children[j].icon+'"></i>';
                       		ulHtml += '<i class="fa ' + data[i].children[j].icon +'"></i>';
                        }else{
...
//                      ulHtml += '<i class="layui-icon" data-icon="'+data[i].icon+'">'+data[i].icon+'</i>';
	...
	Tab.prototype.render = function(data) {
		//显示左侧菜单
		var _this = this;
		$(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="'+data[0].href+'"><i class="fa ' + data[0].icon +'"></i><cite>'+data[0].title+'</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height()-210);
	...
	$(".closePageOther").on("click",function(){
		var navtxt = navs[0].title;
		if($("#top_tabs li").length>2 && $("#top_tabs li.layui-this cite").text()!= navtxt){
			var menu = JSON.parse(window.sessionStorage.getItem("menu"));
		...
		}else if($("#top_tabs li.layui-this cite").text()== navtxt && $("#top_tabs li").length>1){
			$("#top_tabs li").each(function(){
				if($(this).attr("lay-id") != '' && !$(this).hasClass("layui-this")){
					element.tabDelete("bodyTab",$(this).attr("lay-id")).init();
